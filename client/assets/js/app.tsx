...
export default function App() {
    ...
    function buildQueryString(
        page: number,
        locationOptions: ValueLabelId | null,
        roleSearch: string,
        remoteFilter: boolean,
        experienceFilter: MultiValue<ValueLabel> | null,
        sortOption: SingleValue<ValueLabel>
    ): string {
        let jobs_params = `&p=${page}`

        if (locationOptions) jobs_params += `&location=${locationOptions?.value}`

        if (roleSearch) jobs_params += `&role=${roleSearch}`

        if (remoteFilter) jobs_params += `&remote=${remoteFilter}`
        ...
        if (sortOption) jobs_params += `&sort=${sortOption.value}`
        ...
        <>
            ...
            <div className="flex flex-col lg:flex-row py-0">
                <div className="basis-3/3 lg:basis-2/3">
                    ...
                    <div className="w-full">
                        <input
                            name="myInput"
                            value={roleSearch}
                            placeholder="'Python' or 'Devops' or 'AWS'..."
                            autoComplete="off"
                            onChange={e => {
                ...
                <div className="basis-1/3">
                    ...
                    <div className="pl-20 pt-2 float-right lg:float-none">
                        <ToggleSwitch label="Remote only" checked={remoteFilter} onChange={setRemoteFilter} />
                    </div>
                </div>
            ...
            <div className="hidden lg:block">
                <div className={` ${showMore ? "hidden" : "show"}`}>
                    <button
                        className="pt-4 w-full text-slate-500 focus:outline-none focus:text-slate-700 text-right"
            ...
            <div className={`py-2 ${showMore ? "show" : "hidden"}`}>
                <button
                    className="w-full text-slate-500 focus:outline-none focus:text-slate-700 text-right"
                    onClick={toggleShowMore}
                >
                    Show less
                </button>
                ...
                <table className="table-fixed w-full lg:min-w-full bg-white shadow-md rounded-xl py-6">
                    <thead>
                        <tr>
                            ...
                            <th className="hidden lg:table-cell rounded-lg px-6 py-3 text-xs font-medium leading-4 tracking-wider text-left text-gray-400 uppercase border-b border-gray-200 bg-gray-50"></th>
                        </tr>
                    </thead>
                    <tbody className="text-blue-gray-900">
                        <Fragment>
                            {jobs.map(job => {
                                return (
                                    <tr
                                        className="border-b border-blue-gray-200 hover:bg-gray-50 cursor-pointer"
                                        key={job.id}
                                    >
                                        <td
                                            className="py-3 lg:py-6 px-3 lg:px-6 w-auto lg:w-5/12 h-auto lg:h-[100px]"
                                        ...
                                        >
                                            ...
                                            <div className="text-lg">
                                                ...
                                                <div
                                                    className="hidden lg:inline-block pl-4 inline-block underline text-xs text-blue-600 hover:text-blue-800 visited:text-purple-600"
                                                    onClick={() => window.open(job.careers_page, "_blank")}
                                                >
                                        ...
                                        <td
                                            className="hidden lg:table-cell py-3 lg:py-6 px-6"
                                            onClick={() => window.open(job.source_url, "_blank")}
                                        >
                                            <div className="text-lg">{job.company}</div>
                                            <a
                                                href="https://www.glassdoor.com/?utm_source=gocoderemote.com"
                                                rel="noindex nofollow"
                                                target="_blank"
                                            >
                                                <div className="text-xs text-gray-400 pr-6">
                                                    Glassdoor <RatingLabel rating={job.glassdoor} />
                                                </div>
                                            </a>
                                            <a
                                                href="https://www.indeed.com/?utm_source=gocoderemote.com"
                                                rel="noindex nofollow"
                                                target="_blank"
                                            >
                                                <div className="text-xs text-gray-400 pr-6">
                                                    Indeed <RatingLabel rating={job.indeed} />
                                                </div>
                                            </a>
                                        ...
                                        <td
                                            ...
                                            onClick={() => window.open(job.source_url, "_blank")}
                                        >
                                            <div>{job.location + ", " + (job.state ? job.state : job.country)}</div>
                                        ...
                                        <td className="hidden lg:table-cell py-3 lg:py-6 px-6">
                                            ...
                                            <a
                                                type="submit"
                                                className="px-3 py-2 text-lg font-medium text-center text-white bg-gray-400 rounded-lg hover:bg-gray-500 focus:ring-4 focus:outline-none focus:ring-blue-300"
                                                href={job.source_url}
                                                target="_blank"
                                                rel="noindex nofollow"
                                            >
            ...
            <div ref={bottom} className="text-center block w-full h-[100px]"></div>
        </>
    )
}
